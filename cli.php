...
class WP_Redis_CLI_Command
{
    ...
    public function enable()
    {
        if (defined('WP_REDIS_OBJECT_CACHE') && WP_REDIS_OBJECT_CACHE) {
            WP_CLI::success('LaRuta Redis is already enabled.');
            return;
        }
        ...
        if (symlink($object_cache, 'object-cache.php')) {
            ...
            WP_CLI::error('Failed create wp-content/object-cache.php symlink and enable LaRuta Redis.');
        }
    }
    ...
    public function info($_, $assoc_args)
    {
        ...
        if (!defined('WP_REDIS_OBJECT_CACHE') || !WP_REDIS_OBJECT_CACHE) {
            WP_CLI::error('LaRuta Redis object-cache.php file is missing from the wp-content/ directory.');
        }

        if ($wp_object_cache->can_redis() && WP_CLI\Utils\get_flag_value($assoc_args, 'reset')) {
...
WP_CLI::add_command('lrt-redis', 'WP_Redis_CLI_Command');